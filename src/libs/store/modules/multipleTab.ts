...
import { MULTIPLE_TABS_KEY } from '/@/libs/enums/cacheEnum';
import { PageEnum } from '/@/libs/enums/pageEnum';
import { REDIRECT_ROUTE } from '/@/libs/router/routes';
import { getRawRoute } from '/@/libs/utils';
...
export const useMultipleTabStore = defineStore({
  ...
  actions: {
    ...
    goToPage(router: Router) {
      ...
      let toPath: PageEnum | string = PageEnum.MAIN_HOME;
    ...
    async addTab(route: RouteLocationNormalized) {
      ...
      if (path === PageEnum.ERROR_PAGE || path === PageEnum.BASE_LOGIN || !name || [REDIRECT_ROUTE.name].includes(name as string)) {
        return;
      }
    ...
    async closeTab(tab: RouteLocationNormalized, router: Router) {
      const close = (route: RouteLocationNormalized) => {
        const { fullPath, meta: { affix } = {} as any } = route;
        if (affix) {
          return;
        }
    ...
    async closeTabByKey(key: string, router: Router) {
      ...
      if (index !== -1) {
        ...
        if (isActivated === -1) {
          ...
          if (pageIndex >= 0) {
            const page = this.tabList[pageIndex];
