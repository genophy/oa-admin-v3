...
import { ContentTypeEnum } from '/@/libs/enums/httpEnum';
...
export class VAxios {
  ...
  private setupInterceptors() {
    ...
    responseInterceptorsCatch && isFunction(responseInterceptorsCatch) && this.axiosInstance.interceptors.response.use(undefined, responseInterceptorsCatch as any);
  }

  /**
   * 上传文件
   * @param config
   * @param params {data:{name:'',id:'',otherField:''}}
   * @param options
   * @returns
   */
  uploadFile<T = any>(config: AxiosRequestConfig, params: UploadFileParams, options: RequestOptions = {}): Promise<T> {
    // options.isTransformResponse = options?.isTransformResponse ?? false;
  ...
  postFormData<T = any>(config: AxiosRequestConfig, params: UploadFileParams) {
    const formData = new window.FormData();
    if (params.data) {
