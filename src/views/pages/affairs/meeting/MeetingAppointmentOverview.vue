<!--
  FileDesc  : MeetingAppointmentOverview 会议预约
  path      : src/views/pages/affairs/meeting/MeetingAppointmentOverview.vue
  Author    : genophy
  Version   :
  Usage     :
-->
<template>
  <BaseQueryDataPagination
    ...
    class="h-full v-meeting-appointment-overview"
  ...
  >
    ...
    <template #ops>
      <div class="flex px-4 pb-2 items-center justify-end">
        <el-button text type="success" @click="handleViewMyMeetingAppointments">
      ...
      <div class="flex">
        ...
        <div>
          ...
          <BaseCard v-for="item in list" :key="item" class="h-40 m-2 text-xs w-40">
            <div class="flex font-bold text-sm"> <BaseIcon icon="ion:logo-windows" class="mr-2" />会议室 {{ item }}</div>
            ...
            <div class="flex flex-nowrap">
              <div v-for="(_day, idx) in days" :key="idx" class="border flex-shrink-0 h-40 mx-2 text-xs p-2 w-50 overflow-auto">
                <div class="flex mb-2 justify-end">
                  <ElButton link @click="handleAddMeetingAppointment(item)">
                    <template #icon>
...
import CompMeetingAppointment from './components/CompMeetingAppointment.vue';
import CompMyMeetingAppointments from './components/CompMyMeetingAppointments.vue';
 ...
 * 查看所有我的预约
 */
const handleViewMyMeetingAppointments = () => {
  dynDialog.open(CompMyMeetingAppointments, {}, { title: '我的预约会议', width: '600px' }).then((res) => {
 ...
 * 新增会议预约
 */
const handleAddMeetingAppointment = (meetingRoom) => {
  dynDialog.open(CompMeetingAppointment, { meetingRoom }, { title: '新增预约', width: '600px' }).then((res) => {
    if (res.type === 'ok') {
      console.log('你点击了确定');
