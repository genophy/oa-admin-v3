<!--
  FileDesc  : CompCarAppointment 预约会议
  path      : src/views/pages/affairs/car/components/CompCarAppointment.vue
  Author    : genophy
  Version   :
  Usage     :
-->
<template>
  <div class="flex flex-col v-comp-car-appointment">
      ...
      <div class="flex">
        ...
        <div class="border-l px-4 pb-2 overflow-hidden">
          <div class="mb-2">
            ...
            <span class="font-extrabold text-base">{{ carRoom.name }}</span>
          </div>
          <div class="text-xs leading-6">
            <div class="flex">
              <span class="flex-1">房号：{{ carRoom.roomNumber || '-' }}</span>
              <span class="flex-1">容纳人数：{{ carRoom.maxPersonCount || '-' }}</span>
            </div>
            <div class="break-all line-clamp-2">配置：{{ carRoom.equipmentIntro || '-' }}</div>
            <div class="truncate">地址：{{ carRoom.address || '-' }}</div>
          </div>
        </div>
      </div>
      <div class="mt-4 px-8">
...
const formRules = reactive<FormRules>({
  ...
  carRoomId: [{ required: true, message: '请选择会议室', trigger: 'change' }],
...
const handleSubmit = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid, fields) => {
    if (valid) {
      ...
      //     // 编辑
      //     return doEditCar(unref(formField));
      //   } else {
      //     // 新增
      //     return doAddCar(unref(formField));
      //   }
      // }).then((_action) => {});
    } else {
